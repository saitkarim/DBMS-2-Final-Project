...
public class HPController implements Initializable {
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {
            ...
            String query = "SELECT brand, laptop_name, display_size, processor_type, graphics_card, disk_space FROM laptops WHERE brand = 'HP'";
       ...
       laptops_table.setItems(observableList);
        ...
        searchField.setOnKeyReleased( e->  {
            searchField.textProperty().addListener((observable, oldValue, newValue) -> {
               /* filteredList.setPredicate(laptops_info -> {
                    ...
                    if(laptops_info.getBrand().toLowerCase().indexOf(lowerCaseFilter) != -1){
                        return true;
                    } else if(laptops_info.getLaptop_name().toLowerCase().indexOf(lowerCaseFilter) != -1){
                        return true;
                    } else if(laptops_info.getDisplay_size().toLowerCase().indexOf(lowerCaseFilter) != -1){
                        return true;
                    } else if(laptops_info.getProcessor_type().toLowerCase().indexOf(lowerCaseFilter) != -1){
                        return true;
                    } else if(laptops_info.getGraphics_card().toLowerCase().indexOf(lowerCaseFilter) != -1){
                        return true;
                    } else if(laptops_info.getDisk_space().toLowerCase().indexOf(lowerCaseFilter) != -1){
                        return true;
                    } else
                        return false;
